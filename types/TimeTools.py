...
class TimeTools:
    ...
    def run(self, bv: BinaryView) -> bool:
        if not self.find_format_real_time_seconds(bv):
            log_warn("Unable to find jag::game::TimeTools::FormatRealTimeSeconds")

        eastl_basic_string_range_intialize_func = None
        for insn in self.format_real_time_seconds_func.llil.instructions:
    ...
    def find_format_real_time_seconds(self, bv: BinaryView) -> bool:
        for string in bv.strings:
            if string.value == '%H:%M:%S':
                ...
                if xrefs[0].function.start == xrefs[1].function.start:
                    self.format_real_time_seconds_func = xrefs[0].function
                    log_info(
                        'Found jag::game::TimeTools::FormatRealTimeSeconds @ ' + hex(
                            self.format_real_time_seconds_func.start))
                break
        if self.format_real_time_seconds_func is None:
            return False
        change_func_name(self.format_real_time_seconds_func, "jag::game::TimeTools::FormatRealTimeSeconds")
        return True
